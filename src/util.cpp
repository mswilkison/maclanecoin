...
#else
    const char* pszModule = "maclanecoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "maclanecoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No maclanecoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override maclanecoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "maclanecoin.pid"));
